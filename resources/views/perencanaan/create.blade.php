    ...
    <div class="page-header">
        <div class="page-header-left d-flex align-items-center flex-wrap">
            <div class="page-header-title">
                <h5 class="m-b-10 mb-0">Tambah Perencanaan</h5>
            </div>
            ...
            <div class="col-lg-12">
                ...
                <div class="card table-card">
                    <div class="card-header">
                        <h5 class="mb-0">
                            <i class="feather-plus-circle me-2 text-primary"></i>Form Tambah Perencanaan
                        </h5>
                    </div>
                    <div class="card-body">
                        @if($errors->any())
                        ...
                        <form action="{{ route('perencanaan.store') }}" method="POST" id="perencanaanForm">
                            @csrf
                            ...
                            <div id="detailContainer" class="detail-container mb-4">
                                @php $detailCount = max(1, count(old('perencanaan', []))); @endphp
                                @for($i = 0; $i < $detailCount; $i++)
                                <div class="detail-item card border mb-3" id="detail-{{ $i }}">
                                    <div class="card-header bg-light py-2 d-flex justify-content-between align-items-center">
                                        <span class="fw-semibold">Rencana #{{ $i + 1 }}</span>
                                        @if($i > 0)
                                            <button type="button" class="btn btn-sm btn-outline-danger btn-hapus" data-id="{{ $i }}">
                                                <i class="feather-trash-2 me-1"></i> Hapus
                                            </button>
                                        @else
                                            <button type="button" class="btn btn-sm btn-outline-secondary" disabled>
                                                <i class="feather-minus-circle me-1"></i> Minimal 1
                                            </button>
                                        @endif
                                    </div>
                                        ...
                                        <div class="row g-3">
                                            ...
                                            <div class="col-md-6">
                                                ...
                                                <textarea class="form-control" name="deskripsi[]" rows="3"
                                                          placeholder="Deskripsi lengkap perencanaan...">{{ old('deskripsi.' . $i) }}</textarea>
                                            </div>
                                            <div class="col-md-6">
                                                <label class="form-label">Pelaksanaan</label>
                                                <textarea class="form-control" name="pelaksanaan[]" rows="3"
                                                          placeholder="Rencana pelaksanaan kegiatan...">{{ old('pelaksanaan.' . $i) }}</textarea>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="d-flex align-items-start gap-3">
                                    ...
                                    <div>
                                        ...
                                        <ul class="mb-0 mt-1 ps-3">
                                            <li>Minimal harus ada 1 detail perencanaan</li>
                                            <li>Kolom dengan tanda <span class="text-danger">*</span> wajib diisi</li>
                                            <li>Klik "Tambah Detail" untuk menambahkan rencana baru</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="d-flex flex-wrap justify-content-between align-items-center gap-3">
                                ...
                                <div class="d-flex gap-2">
                                    ...
                                    <button type="submit" class="btn btn-primary" id="submitBtn">
                                        <i class="feather-save me-2"></i>Simpan Perencanaan
                                    </button>
                                </div>
                            </div>
                        </form>
...
document.addEventListener('DOMContentLoaded', function() {
        ...
        const detailHtml = `
            <div class="detail-item card border mb-3 new-added" id="detail-${newIndex}">
                <div class="card-header bg-light py-2 d-flex justify-content-between align-items-center">
                    ...
                    <button type="button" class="btn btn-sm btn-outline-danger btn-hapus" data-id="${newIndex}">
                        <i class="feather-trash-2 me-1"></i> Hapus
                    </button>
                </div>
                <div class="card-body py-3">
    ...
    document.addEventListener('click', function(e) {
        ...
        if (deleteBtn) {
            ...
            Swal.fire({
                ...
                reverseButtons: true
            }).then((result) => { if (result.isConfirmed) deleteDetail(id); });
        }
    });
    ...
    document.getElementById('resetBtn')?.addEventListener('click', function(e) {
        e.preventDefault();
        Swal.fire({
            title: 'Reset Form?', text: 'Semua data yang sudah diisi akan hilang. Lanjutkan?', icon: 'warning',
            showCancelButton: true, confirmButtonColor: '#ffc107', cancelButtonColor: '#6c757d',
            confirmButtonText: '<i class="feather-refresh-cw me-2"></i>Ya, Reset', cancelButtonText: '<i class="feather-x me-2"></i>Batal'
        }).then((result) => {
            if (result.isConfirmed) { form.reset(); location.reload(); }
        });
    });
});
